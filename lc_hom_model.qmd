...
$$
  u(c_i^h(s)) + \delta \sum_{s'} \pi(s' | s) V_i^{h - 1} (s', x'_i(s', \underline{x}^h(s))) = U_i^{aut}(s),
$$ 
...
$$
  u(c_v^h(s)) + \delta \sum_{s'} \pi(s' | s) V_v^{h - 1} (s', x'(s', \overline{x}^h(s))) = U_v^{aut}(s),
$$ 
where the optimality condition is
$$
  \frac{u'(c_{2}^h(s))}{u'(c_{1}^h(s))} = \overline{x}^h(s).
$$
The next-period relative Pareto weight, $x'$, is determined by the interlval-based update rule, where $\underline{x}^{h - 1}(s)$ and $\underline{x}^{h - 1}(s)$ are used as the interval limits.

<!-- Notice that, once the PC binds, the past history, which is summarized by $x_{t - 1}$, does not matter. This property is called "amnesia" (@Kocherlakota1996) or "forgiveness" (@Ligon2002).  -->
Since $\underline{x}^h(s)$ may not be on the grid $q$, linear interpolation is used to compute $V_i^{h - 1}(s', \underline{x}^h(s))$.
...
interpolateValueFunction <- function(
  ...
  map_dbl(
    seq(1, numIncomeStates),
    function(x) {
      approx(
        relativeParetoWeightsGridPoints,
        valueFunctionMatrix[x, ],
        relativeParetoWeightVec[x],
        rule = 2
      )$y
    }
  )
...
calculateDiffLCRiskSharingAndAutarky <- function(
  ...
  nextPeriodRelativeParetoWeightVec <- map_dbl(
    seq(1, numIncomeStates),
    function(x) {relativeParetoWeight %>% 
      pmax(relativeParetoWeightBounds[x, 1]) %>% 
      pmin(relativeParetoWeightBounds[x, 2])}
  )
  
  householdValueLCRiskSharingAtRelativeParetoWeight <- interpolateValueFunction(
    nextPeriodRelativeParetoWeightVec,
    relativeParetoWeightsGridPoints,
    householdValueLCRiskSharing,
    numIncomeStates
    )
...
calculateValueLCRiskSharing <- function(
  ...
  relativeParetoWeightBounds <- cbind(
    rep(min(relativeParetoWeightsGridPoints), numIncomeStates),
    rep(max(relativeParetoWeightsGridPoints), numIncomeStates)
  )
  relativeParetoWeightBoundsNew <- matrix(NA, nrow = numIncomeStates, ncol = 2)
  ...
  # Next derive lower and upper bounds of relative Pareto weight intervals
  diff = 1
  iteration = 1
  while ((diff > diffLimit) && (iteration <= iterationLimit)) {
    for (incomeStateIndex in seq(1, numIncomeStates)) {
      ...
      relativeParetoWeightBoundsNew[incomeStateIndex, 1] <- relativeParetoWeightLowerBound
      relativeParetoWeightBoundsNew[incomeStateIndex, 2] <- relativeParetoWeightUpperBound
    }
    
    diff <- max(
      abs(relativeParetoWeightBoundsNew - relativeParetoWeightBounds)
      )
    relativeParetoWeightBounds <- relativeParetoWeightBoundsNew
    iteration <- iteration + 1
  }
...
tic()
tmp2 <- solveLCRiskSharingByVillage(
    village,
    delta,
    sigma,
    punishment,
