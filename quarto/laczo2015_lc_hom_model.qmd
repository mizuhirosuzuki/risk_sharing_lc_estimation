...
calculateValueLCRiskSharing <- function(
  ...
  for (incomeStateIndex in seq(1, numIncomeStates)) {
    ...
    c(min(relativeParetoWeightsGridPoints) - 1e-6, max(relativeParetoWeightsGridPoints)), 
    tol = 1e-10, 
    maxiter = 300
    )$root
    ...
    c(min(relativeParetoWeightsGridPoints), max(relativeParetoWeightsGridPoints) + 1e-6), 
    tol = 1e-10, 
    maxiter = 300
    )$root
