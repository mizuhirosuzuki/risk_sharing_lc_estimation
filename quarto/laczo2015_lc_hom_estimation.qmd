...
#       dnorm(
#         log(householdCons) -
#         log(calculateHouseholdConsumption(
#           aggregateIncomeTrue,
...
calculateLogLikelihoodLCHom <- function(
  ...
  for (simulationIndex in seq(1, numSimulations)) {
    ...
    for (householdIndex in seq(1, numHouseholds)) {
      for (periodIndex in seq(2, tnum)) {
        ...
        # villageIncomeTrue <- exp(log(incTrue) %>% colMeans(na.rm = TRUE))[periodIndex]
  ...
  for (simulationIndex in seq(1, numSimulations)) {
    ...
    for (householdIndex in seq(1, numHouseholds)) {
      for (periodIndex in seq(2, tnum)) {
        logConsPredict <- (
          log(
            (incTrue[, periodIndex] %>% sum(na.rm = TRUE)) 
            / (
        ...
        likelihoodArray[householdIndex, periodIndex - 1, simulationIndex] <- dnorm(
  ...
  for (householdIndex in seq(1, numHouseholds)) {
    for (periodIndex in seq(2, tnum)) {
      ...
      # if (!is.nan(likelihoodSimMean) & (likelihoodSimMean > 0)) {
      if (!is.nan(likelihoodSimMean)) {
        logLikelihood <- (
          logLikelihood 
          # + log(likelihoodSimMean %>% pmax(1e-16))
          + log(likelihoodSimMean)
        )
      }
    }
  }
...
tic()
calculateLogLikelihoodLCHom(
  c(0.8, 1.5, 0.3, 0.05, 0.09),
  2,
  consdat,
  incdatRescaled,
  # incdatRescaledWinsorized,
  householdIncMeanClassVec,
  householdIncCVClassVec,
  householdAR1EstimationResult,
  villageAR1EstimationResult,
...
lowerBounds <- c(0.5, 1.0, 0.0, 1e-3, 1e-3)
upperBounds <- c(0.98, 5.0, 0.99, 1.0, 1.0)

tic()
tmp2 <- estimateMLLCHom(
  initialParam,
  lowerBounds,
  upperBounds,
  2,
