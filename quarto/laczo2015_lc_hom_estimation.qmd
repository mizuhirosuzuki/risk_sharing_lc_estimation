...
To explain the discrepancies from the observed consumption and the predicted consumption, I introducemultiplicative measurement errors in consumption, as in the full risk-sharing model.
...
1. Randomly generate measurement errors, and denote their $s$'th simulations for $i$ at $t$ by $\varepsilon_{s, i, t}^c$.
2. Calculate the previous-period relative Pareto weight by $x_{s, t - 1} = \frac{c_{v, t - 1}^{*-\sigma} / \exp(\varepsilon_{s, v, t - 1}^c)}{c_{i, t - 1}^{*-\sigma} / \exp(\varepsilon_{s, i, t - 1}^c)}$.
