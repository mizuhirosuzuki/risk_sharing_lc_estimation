  ...
  for (simulationIndex in seq(1, numSimulations)) {
    ...
    for (householdIndex in seq(1, numHouseholds)) {
      for (periodIndex in seq(2, tnum)) {
        ...
        householdIncomeTrue <- incdatRescaled[householdIndex, periodIndex]
        villageIncomeTrue <- (incdatRescaled %>% colMeans(na.rm = TRUE))[periodIndex]
  ...
  for (simulationIndex in seq(1, numSimulations)) {
    ...
    for (householdIndex in seq(1, numHouseholds)) {
      for (periodIndex in seq(2, tnum)) {
        logConsPredict <- (
          log(
            (incdatRescaled[, periodIndex] %>% sum(na.rm = TRUE)) 
            / (
  ...
  for (householdIndex in seq(1, numHouseholds)) {
    for (periodIndex in seq(2, tnum)) {
      ...
      if (!is.nan(likelihoodSimMean)) {
        logLikelihood <- (
          logLikelihood 
          + log(likelihoodSimMean %>% pmax(1e-8))
        )
      }
    }
  }
