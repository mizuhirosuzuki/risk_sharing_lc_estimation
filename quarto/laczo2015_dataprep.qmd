...
In this page, I demonstrate the data prep process. The main goal in the data prep is to estimate income processes of households and the "village". For this, I discretize the continuous income transition processes and approximate the autoregressive process.
...
vilSumIncByYear <- createVillageAggregateByYear(incdat, colSums, numVillages, villageIndicatorMatrix)
vilMeanInc <- createVillageAggregate(incdat, mean, numVillages, villageIndicatorMatrix)
...
vilSumConsByYear <- createVillageAggregateByYear(consdat, colSums, numVillages, villageIndicatorMatrix)
vilMeanCons <- createVillageAggregate(consdat, mean, numVillages, villageIndicatorMatrix)
```
...
consLow <- createVillageAggregate(
  consdat, 
  function(x) quantile(x, 0.025, na.rm = TRUE),
  numVillages, 
  villageIndicatorMatrix
  )
consHigh <- createVillageAggregate(
  consdat, 
  function(x) quantile(x, 0.975, na.rm = TRUE),
  numVillages, 
  villageIndicatorMatrix
...
We want the income process of the village since, in estimation, we consider the risk-sharing transfers between a household and the village so that we can consider one-to-one transfers instead of on-to-n transfers.

For this, first I estimate the income process of households by types, where the types are by income's mean and coefficient of variance (CV).
Here, in total we have 4 types (high/low mean income X high/low CV income).
If there were long enough income data, we would have been able to estimate the income process of each household, but given the 6 observations per household for income in ICRISAT data, the author took this approach.
...
This estimation process is detailed in [the paper's online appendix](https://bf7cb7e7-b25e-4ff4-a178-ceeb7e0cc1cf.filesusr.com/ugd/7fa395_de0f195df4b642cb86930520007484c9.pdf)
...
For this, the author used Tauchen's method with a small modification that, instead of assigning the bounds of grid points with a parameter, the quantiles of income distributions are used to determine each grid point.
...
Given the simulated income data, I estimate the income process parameters and apprximate the process with Tauchen's method.
...
## Sanity check: compare against the parameters in the original paper

Just for a sanity check of my code, I compare the household AR(1) process parameters I derived against the ones provided in the appendix of the paper.
The parameters in the tables below coincide to the parameters in the paper appendix, excpet the ones for "Low mean, high risk" in Shirapur.
